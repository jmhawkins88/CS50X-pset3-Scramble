...
void scramble(void)
{
    //create temporary new grid
    char newgrid[DIMENSION][DIMENSION];

    //iterates through grid and copies rotated values into new grid
    for (int i = 0; i < DIMENSION; i++)
    {
        for (int j = 0; j < DIMENSION; j++)
        {
            //rotates grid 90 degrees and copies to new grid
            newgrid[i][j] = grid[i][j];
        }
    }

    //iterates through new grid and copies values to old grid
    for (int i = 0; i < DIMENSION; i++)
    {
        for (int j = 0; j < DIMENSION; j++)
        {
            grid[i][j] = newgrid[DIMENSION - 1 - j][i];
        }
    }
}
